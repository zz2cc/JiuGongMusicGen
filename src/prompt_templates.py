...
# ============================================================================
# Bare Prompt — 极简提示词（无曲牌特征/声调规则/样例/风格指导）
# ============================================================================

BARE_SYSTEM_PROMPT = """你是中国传统音乐专家。请为歌词生成工尺谱旋律。

格式规则：
- 每个字后紧跟[工尺...]，多个工尺用空格分隔
- 工尺序列（从低到高）: 合 四 一 上 尺 工 凡 六 五 乙 仩 伬 仜
- 时长标记: 无标记=四分, (O)=二分, (x)=八分
- 只输出"旋律: 字[工尺] 字[工尺]..."一行，不要其他内容"""


def build_bare_task(qupai: str, lyrics: str) -> str:
    """构建极简用户提示词（不含曲牌特征/样例/声调/风格）"""
    lines = [
        f"曲牌: 《{qupai}》",
        f"歌词: {lyrics}",
        "",
